...
-- Match MollyThePoly param names from molly_the_poly_engine.lua
-- ============================================================================
...
local state = {
  root_note = 60,              -- C4 (MIDI note, already includes octave)
...
-- root_note already includes octave info (e.g. 60 = C4)
-- so we just add intervals directly — no extra octave multiplication
local function build_chord(root, chord_type_name)
...
local function start_arp(row, notes)
  ...
  state.arp_clocks[row] = clock.run(function()
    while state.arp_active[row] and #state.arp_notes[row] > 0 do
      ...
      -- div values are fractions of a measure; multiply by 4 to get beats
      local beats = div * 4
      clock.sync(beats)
...
local function grid_key(x, y, z)
  if z == 0 then
    ...
    if x <= 15 and state.held_chords[y] and state.held_chords[y][x] then
      ...
      -- If arp not active for this row, nothing more to do
    ...
    if x <= 15 then
      ...
      local notes = build_chord(chord_root, chord_type)
...
function enc(n, d)
  ...
  elseif n == 2 then
    -- Root note
    state.root_note = clamp(state.root_note + d, 36, 84)  -- C2 to C6
  elseif n == 3 then
    -- Chord type
